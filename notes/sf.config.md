...
The attachment graph, not the `_config` file location by itself, gives a config payload authority for a layer. Mesh-local source attachments are authored on the active `sflo:SemanticMesh` subject, normally in `_mesh/_meta/meta.ttl`. Knop-local and Knop-inheritable source attachments are authored on the current `sflo:Knop` subject, normally in `<knop>/_knop/_meta/meta.ttl`. A source can resolve bytes from `_mesh/_config/config.ttl`, `<knop>/_knop/_config/config.ttl`, or another governed config artifact, but the resolved `_config/*.ttl` file is the config payload and provenance, not the initial bootstrap authority.

Within the attachment graph, `hasConfig` / `hasConfigSource` mean local config for the attachment subject, while `hasInheritableConfig` / `hasInheritableConfigSource` mean Knop-authored defaults for descendant Knop scopes. Mesh config has no separate mesh-inheritable modality; mesh-level policy can cover Knop-governed targets through ordinary selectors and scope. A config artifact that is already reached through an authorized source may chain to other config sources when the implementation supports config-source chaining, but a config file should not be treated as visible merely because it contains a self-declaring `hasConfigSource` triple.
