...
`payload.update` is still better understood as a local convenience mutation surface. Replacing current working bytes is real implementation work, but the semantically meaningful lifecycle step remains `version` or `weave`, which records those bytes into explicit artifact history.
